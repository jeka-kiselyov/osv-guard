 ...
 */
export function resolveProjectDir(flagDir, env = process.env, cwd = process.cwd()) {
    if (flagDir)
        return { dir: path.resolve(cwd, flagDir), via: 'flag' };
...
const LOCKFILE_SET = new Set(LOCKFILES);
 ...
 */
export function findLockfiles(dir) {
    const found = [];
    const walk = (current, depth) => {
        if (depth > MAX_DEPTH || found.length >= MAX_LOCKFILES)
            return;
        let entries;
        try {
