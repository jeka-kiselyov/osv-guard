...
const MATCHERS = [
    ...
    ...PYTHON_INSTALLERS.map(({ verb }) => ({ pattern: verb, ecosystem: 'PyPI' })),
];
/**
 ...
 */
export function isPackageArgument(arg) {
    if (arg.startsWith('-'))
        return false;
    ...
    if (/\.(?:tgz|tar\.gz|whl|zip)$/i.test(arg))
        return false;
...
export function parseSpec(arg, ecosystem) {
    if (!isPackageArgument(arg))
        return null;
    if (ecosystem === 'PyPI') {
