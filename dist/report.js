...
import { createColors, padEnd } from './colors.js';
import { BAND_RANK } from './types.js';
import { VERSION } from './version.js';
const BAND_ORDER = { ...BAND_RANK, unknown: 0 };
const BAND_LABEL = {
    critical: 'CRITICAL',
    high: 'HIGH',
    moderate: 'MODERATE',
...
function paint(colors, band, text) {
    switch (band) {
        case 'critical':
...
export function renderSummary(ctx, colors) {
    const c = ctx.policy.counts;
    if (c.total === 0)
        return colors.green('osv-guard: no known vulnerabilities');
    const parts = ['critical', 'high', 'moderate', 'low', 'unknown']
...
export function renderPretty(ctx, colors) {
    ...
    lines.push('');
    lines.push(`${colors.bold('osv-guard')} ${colors.gray('·')} scanned ${colors.bold(rel(dir))} ${colors.gray(`(${scanned})`)} ${timing}`);
    lines.push('');
    ...
    if (c.total === 0) {
        ...
        const tally = ['critical', 'high', 'moderate', 'low', 'unknown']
            .filter((band) => c[band] > 0)
        ...
        for (const f of sorted) {
            ...
            const score = f.score !== null ? colors.gray(f.score.toFixed(1).padStart(4)) : colors.gray(' —  ');
            ...
            lines.push(`${indent}${f.fixedVersion
                ? colors.green(`→ fixed in ${f.fixedVersion}`)
                : colors.yellow('→ no fix published')}  ${colors.gray(f.url)}`);
        }
    }
    lines.push('');
    ...
    if (policy.ignoredIds.length > 0) {
        lines.push(colors.gray(`  ${policy.ignoredIds.length} finding(s) suppressed by --ignore: ${policy.ignoredIds.join(', ')}`));
    }
    if (policy.unusedIgnores.length > 0) {
        lines.push(colors.yellow(`  ! ignore entries that matched nothing (stale?): ${policy.unusedIgnores.join(', ')}`));
    }
    if (c.unknown > 0 && !options.failOnUnknown) {
        lines.push(colors.gray(`  ${c.unknown} finding(s) have no usable severity and were not gated — use --fail-on-unknown to block them.`));
    }
 ...
 */
export function describeLockfiles(lockfiles) {
...
export function findingLocation(finding, dir) {
    if (!finding.source)
        return '';
