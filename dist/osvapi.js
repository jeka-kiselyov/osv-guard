 ...
 */
export async function queryPackage(spec, { fetchImpl = globalThis.fetch, timeoutMs = 5000 } = {}) {
    const body = {
    ...
    try {
        ...
        return { spec, vulns: [], error: err.message || 'request failed' };
    }
}
export async function queryAll(specs, options = {}) {
 ...
 */
export function isMalicious(vuln) {
    if (vuln.id.startsWith('MAL-'))
        return true;
    if (vuln.database_specific && 'malicious-packages-origins' in vuln.database_specific)
        return true;
 ...
 */
export function toScanOutput(results) {
    return {
        results: results
