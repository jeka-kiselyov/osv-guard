...
const LOCKFILE_PM: ReadonlyArray<readonly [string, PackageManager]> = [
  ['pnpm-lock.yaml', 'pnpm'],
  ['yarn.lock', 'yarn'],
  ['bun.lock', 'bun'],
  ['bun.lockb', 'bun'],
...
function asPm(value: string | undefined): PackageManager | null {
...
function fromLockfile(dir: string): PackageManager | null {
  ...
  for (;;) {
    for (const [name, pm] of LOCKFILE_PM) {
      if (existsSync(path.join(current, name))) return pm;
    }
...
function fromUserAgent(env: NodeJS.ProcessEnv): PackageManager | null {
...
export function detectPackageManager(
  ...
  if (flag) {
    const pm = asPm(flag);
    if (!pm) throw new Error(`unknown package manager: ${flag}`);
    return { pm, via: 'flag' };
  }

  const declared = fromPackageManagerField(dir);
  if (declared) return { pm: declared, via: 'packageManager' };

  const lock = fromLockfile(dir);
  if (lock) return { pm: lock, via: 'lockfile' };

  const agent = fromUserAgent(env);
  if (agent) return { pm: agent, via: 'user-agent' };
 ...
 */
export function buildRunArgs(pm: PackageManager, script: string, args: string[]): string[] {
  if (args.length === 0) return ['run', script];
  if (pm === 'npm') return ['run', script, '--', ...args];
  return ['run', script, ...args];
}

/** npm and yarn ship as `.cmd` shims on Windows, which need a shell to launch. */
export function pmExecutable(pm: PackageManager): { command: string; shell: boolean } {
