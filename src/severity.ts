...
export function scoreToBand(score: number): Band | null {
...
export function normalizeBandName(raw: string | undefined | null): Band | null {
...
export function resolveSeverity(
  ...
  databaseSpecific: string | undefined,
  vectors: OsvRawSeverity[] | undefined,
): SeverityResolution {
  if (maxSeverity !== undefined && maxSeverity !== '') {
    const score = Number(maxSeverity);
    const band = scoreToBand(score);
    if (band) return { band, score, source: 'max_severity' };
